...
local function get_forvo_audio(word)
	...
	if platform == "windows" then
      ...
      $data = Invoke-WebRequest -Uri "https://forvo.com/search/]]
				.. url_enc(word)
				.. [[/ja/" -Headers @{
      "method"="GET"
      "authority"="forvo.com"
      ...
      [Console]::OpenStandardOutput().Write($u8data, 0, $u8data.Length)
		...
		args = {
			"curl",
			"https://forvo.com/search/" .. word .. "/ja/",
			"-H",
			"authority: forvo.com",
			"-H",
			"pragma: no-cache",
			...
			"referer: https://forvo.com",
			"-H",
			"accept-language: en-US,en;q=0.9,ny;q=0.8,ja;q=0.7,es;q=0.6",
		}
	end
	...
	if platform == "windows" then
		args = {
			...
			[[& {
      [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
      Invoke-WebRequest -Uri "]] .. audio_url .. [[" -OutFile "]] .. utils.join_path(
				prefix,
				"forvo_" .. word .. ".mp3"
			) .. [["
    }]],
